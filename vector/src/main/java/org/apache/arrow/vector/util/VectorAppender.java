...
public class VectorAppender implements VectorVisitor<ValueVector, Void> {
    ...
    public ValueVector visit(BaseFixedWidthVector deltaVector, Void value) {
        Preconditions.checkArgument(
            targetVector
                .getField()
                .getType()
                .equals(deltaVector.getField().getType()),
        ...
        int newValueCount =
            targetVector.getValueCount() + deltaVector.getValueCount();
    ...
    public ValueVector visit(
        BaseLargeVariableWidthVector deltaVector,
        Void value
    ) {
        Preconditions.checkArgument(
            targetVector
        ...
        long targetDataSize = targetVector
            .getOffsetBuffer()
            .getLong(
                (long) targetVector.getValueCount() *
                BaseLargeVariableWidthVector.OFFSET_WIDTH
            );
        long deltaDataSize = deltaVector
            .getOffsetBuffer()
            .getLong(
                (long) deltaVector.getValueCount() *
                BaseLargeVariableWidthVector.OFFSET_WIDTH
        ...
        ((BaseLargeVariableWidthVector) targetVector).setLastSet(
                newValueCount - 1
            );
        targetVector.setValueCount(newValueCount);
        return targetVector;
    ...
    @Override
    public ValueVector visit(
        BaseVariableWidthViewVector deltaVector,
        Void value
    ) {
        Preconditions.checkArgument(
        ...
        for (int i = oldTargetValueCount; i < newValueCount; i++) {
            if (
                ...
                targetViewBuffer.setInt(
                    start,
                    bufferId + oldTargetDataBufferCount
                );
            }
        }
    ...
    public ValueVector visit(ListVector deltaVector, Void value) {
        ...
        int targetListSize = targetVector
            .getOffsetBuffer()
            .getInt(
                (long) targetVector.getValueCount() * ListVector.OFFSET_WIDTH
            );
        int deltaListSize = deltaVector
            .getOffsetBuffer()
            .getInt(
        ...
        VectorAppender innerAppender = new VectorAppender(
    ...
    public ValueVector visit(LargeListVector deltaVector, Void value) {
        ...
        long targetListSize = targetVector
            .getOffsetBuffer()
            .getLong(
                (long) targetVector.getValueCount() *
                LargeListVector.OFFSET_WIDTH
            );
        long deltaListSize = deltaVector
            .getOffsetBuffer()
            .getLong(
                (long) deltaVector.getValueCount() *
                LargeListVector.OFFSET_WIDTH
        ...
        targetListVector
            .getDataVector()
            .setValueCount(checkedCastToInt(targetListSize));
        deltaVector
            .getDataVector()
            .setValueCount(checkedCastToInt(deltaListSize));
    ...
    public ValueVector visit(FixedSizeListVector deltaVector, Void value) {
        ...
        FixedSizeListVector targetListVector =
            (FixedSizeListVector) targetVector;
        ...
        int targetListSize =
            targetListVector.getValueCount() * targetListVector.getListSize();
        int deltaListSize =
            deltaVector.getValueCount() * deltaVector.getListSize();
    ...
    public ValueVector visit(NonNullableStructVector deltaVector, Void value) {
        ...
        NonNullableStructVector targetStructVector =
            (NonNullableStructVector) targetVector;
        int newValueCount =
            targetVector.getValueCount() + deltaVector.getValueCount();
        ...
        for (
            int i = 0;
            i < targetStructVector.getChildrenFromFields().size();
            i++
        ) {
    ...
    public ValueVector visit(UnionVector deltaVector, Void value) {
        ...
        MemoryUtil.copyMemory(
            ...
            targetUnionVector.getTypeBufferAddress() +
            targetVector.getValueCount(),
            deltaVector.getValueCount()
        );
        ...
        for (int i = 0; i < Byte.MAX_VALUE; i++) {
            if (targetTypes.contains(i) || deltaTypes.contains(i)) {
                ...
                if (deltaTypes.contains(i)) {
                    ...
                    targetChild.setValueCount(
                        targetUnionVector.getValueCount()
                    );
                    deltaChild.setValueCount(deltaVector.getValueCount());
    ...
    public ValueVector visit(DenseUnionVector deltaVector, Void value) {
        ...
        DenseUnionVector targetDenseUnionVector =
            (DenseUnionVector) targetVector;
        int newValueCount =
            targetVector.getValueCount() + deltaVector.getValueCount();
        ...
        MemoryUtil.copyMemory(
            ...
            targetDenseUnionVector.getTypeBuffer().memoryAddress() +
            targetVector.getValueCount(),
            deltaVector.getValueCount()
        );
        ...
        for (int i = 0; i < deltaVector.getValueCount(); i++) {
            ...
            ValueVector targetChildVector =
                targetDenseUnionVector.getVectorByType(typeId);
            int offsetBase = targetChildVector == null
                ? 0
                : targetChildVector.getValueCount();
            int deltaOffset = deltaVector.getOffset(i);
            long index =
                (long) (targetVector.getValueCount() + i) *
                DenseUnionVector.OFFSET_WIDTH;

            targetVector
                .getOffsetBuffer()
                .setInt(index, offsetBase + deltaOffset);
        }

        // append child vectors
        for (int i = 0; i <= Byte.MAX_VALUE; i++) {
            ValueVector targetChildVector =
                targetDenseUnionVector.getVectorByType((byte) i);
            ValueVector deltaChildVector = deltaVector.getVectorByType(
                (byte) i
            );
            ...
            if (targetChildVector == null && deltaChildVector == null) {
                ...
                targetDenseUnionVector.registerNewTypeId(
                    deltaChildVector.getField()
                );
                targetChildVector = targetDenseUnionVector.addVector(
                    (byte) i,
                    deltaChildVector
                        .getField()
                        .createVector(targetDenseUnionVector.getAllocator())
                );

                // now we have both child vectors not null, we can append them.
                VectorAppender childAppender = new VectorAppender(
                    targetChildVector
                );
                ...
                TypeEqualsVisitor childTypeVisitor = new TypeEqualsVisitor(
                    targetChildVector,
                    /* check name */false,
                    /* check meta data*/false
                );
                if (!childTypeVisitor.equals(deltaChildVector)) {
                    throw new IllegalArgumentException(
                        "dense union vectors have different child vector types with type id " +
                        i
                    );
                }
    ...
    public ValueVector visit(ExtensionTypeVector<?> deltaVector, Void value) {
        ValueVector targetUnderlying =
            ((ExtensionTypeVector<?>) targetVector).getUnderlyingVector();
        VectorAppender underlyingAppender = new VectorAppender(
            targetUnderlying
        );
